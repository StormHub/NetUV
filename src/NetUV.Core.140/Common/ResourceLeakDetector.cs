...
namespace NetUV.Core.Common
{
    ...
    public class ResourceLeakDetector
    {
        ...
        const string PropTargetRecords = "LeakDetection.targetRecords";
        ...
        static ResourceLeakDetector()
        {
            ...
            {
                Logger.DebugFormat("{0}: {1}", PropLevel, level.ToString().ToLower());
                Logger.DebugFormat("{0}: {1}", PropTargetRecords, TargetRecords);
            }
        }
        ...
        {
            Logger.ErrorFormat(
                "LEAK: {0}.Release() was not called before it's garbage-collected. {1}",
                type, records);
        }
        ...
        protected void ReportUntracedLeak(string type)
        {
            Logger.ErrorFormat("LEAK: {0}.Release() was not called before it's garbage-collected. " +
                ...
                this.resourceType, PropLevel, DetectionLevel.Advanced.ToString().ToLower(), StringUtil.SimpleClassName(this));
        }
