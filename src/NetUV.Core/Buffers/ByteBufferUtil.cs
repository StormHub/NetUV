...
namespace NetUV.Core.Buffers
{
    ...
    {
        static readonly ILog Logger = LogFactory.ForContext(typeof(ByteBufferUtil));

        public static readonly IByteBufferAllocator DefaultAllocator;

        static ByteBufferUtil()
        {
            string allocType = SystemPropertyUtil.Get("Allocator.type", "pooled");
            ...
            if ("unpooled".Equals(allocType, StringComparison.OrdinalIgnoreCase))
            {
                ...
                Logger.DebugFormat("Allocator.type: {0}", allocType);
            }
            ...
            {
                alloc = PooledByteBufferAllocator.Default;
                Logger.DebugFormat("Allocator.type: pooled (unknown: {0})", allocType);
            }
        ...
        public static string ReadString(IByteBuffer buffer, IByteBuffer separator, Encoding encoding)
        {
            ...
            int len = separator.ReadableBytes;
            int readerIndex = buffer.ReaderIndex;
            if (len == 0 || readableBytes < len)
            {
                buffer.SkipBytes(readableBytes);
                return buffer.ToString(readerIndex, readableBytes, encoding);
            }

            int index = IndexOf(separator, buffer);
            if (index < 0)
            {
                buffer.SkipBytes(readableBytes);
                return buffer.ToString(readerIndex, readableBytes, encoding);
            ...
            int length = index - readerIndex;
            buffer.SkipBytes(length);
            return buffer.ToString(readerIndex, length, encoding);
        }
